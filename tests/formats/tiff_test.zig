...
test "Should error on non TIFF images" {
    ...
    const invalid_file = sgi_file.read(&reader.interface, helpers.zigimg_test_allocator);
...
test "TIFF/LE monochrome black uncompressed" {
    ...
    const pixels = try the_bitmap.read(&reader.interface, helpers.zigimg_test_allocator);
